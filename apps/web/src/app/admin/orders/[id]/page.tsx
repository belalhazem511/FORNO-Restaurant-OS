...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [inventoryDisposition, setInventoryDisposition] = useState<"returned_unused" | "prepared_discarded">("prepared_discarded");
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      ...
      <Dialog open={cancelOpen} onOpenChange={setCancelOpen}><DialogContent><DialogHeader><DialogTitle>{order.payment_status === "paid" ? t("reverseAndCancel") : t("cancelOrder")}</DialogTitle></DialogHeader><div className="space-y-3"><Label htmlFor="cancel-reason">{t("cancellationReason")}</Label><Input id="cancel-reason" value={cancelReason} onChange={(event) => setCancelReason(event.target.value)} />{order.inventory_issued_at && <fieldset className="rounded-lg border p-3"><legend className="px-1 font-semibold">{isArabic ? "مصير المكونات المصروفة" : "Issued ingredient disposition"}</legend><label className="flex min-h-11 items-center gap-2"><input type="radio" checked={inventoryDisposition === "returned_unused"} onChange={() => setInventoryDisposition("returned_unused")} />{isArabic ? "مرتجع غير مستخدم — إعادة الرصيد بدقة" : "Returned unused — reverse exact stock"}</label><label className="flex min-h-11 items-center gap-2"><input type="radio" checked={inventoryDisposition === "prepared_discarded"} onChange={() => setInventoryDisposition("prepared_discarded")} />{isArabic ? "تم التحضير/هالك — تسجيل هالك دون استعادة" : "Prepared/discarded — record waste without restoring"}</label></fieldset>}{order.payment_status === "paid" && <p className="text-sm text-muted-foreground">{t("fullReversalNotice")}</p>}{cancelError && <p role="alert" className="text-sm text-destructive">{cancelError}</p>}</div><DialogFooter><Button variant="outline" onClick={() => setCancelOpen(false)}>{tc("cancel")}</Button><Button variant="destructive" disabled={cancelMutation.isPending || cancelReason.trim().length < 3} onClick={() => { setCancelError(""); cancelMutation.mutate({ orderId, idempotencyKey: cancelRequestId, reason: cancelReason, inventoryDisposition: order.inventory_issued_at ? inventoryDisposition : undefined }); }}>{t("confirmCancellation")}</Button></DialogFooter></DialogContent></Dialog>
    </div>
  );
}
