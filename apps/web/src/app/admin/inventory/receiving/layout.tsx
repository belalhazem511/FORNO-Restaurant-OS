...
export default async function ReceivingLayout({ children }: { children: React.ReactNode }) {
  ...
  const assignment = await db.query.staffAssignments.findFirst({
    where: and(eq(staffAssignments.user_id, user.id), eq(staffAssignments.is_active, true)),
  });
  if (!assignment || !hasPermission(assignment.role, "purchase-receipt:view")) notFound();
  return children;
}
