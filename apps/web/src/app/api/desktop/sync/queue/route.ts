  ...
  await db.transaction(async (tx) => {
    for (const raw of body.results as Array<Record<string, unknown>>) {
      ...
      if ((state === "accepted") && raw.result && typeof raw.result === "object" && typeof (raw.result as Record<string, unknown>).revision === "number") {
        const entityType = item.domain === "customers" ? "customer" : item.domain === "products" ? "product" : item.domain === "shifts" ? item.action === "open" || item.action === "close" ? "cashier_shift" : item.action === "drawer_adjust" ? "cash_movement" : "" : "";
