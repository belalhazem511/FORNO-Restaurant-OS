...
export function OfflineReceiptAction({ documentId, label, className = "" }: { documentId: string; label: string; className?: string }) {
  ...
  return <div className={`space-y-2 ${className}`}>
    ...
    {popupBlocked && <p role="alert" className="rounded-lg border border-amber-400 bg-amber-50 p-3 text-sm text-amber-950">Pop-up blocked. Allow pop-ups for SOLO, then reopen this same receipt. / تم حظر النافذة. اسمح بالنوافذ المنبثقة لـ SOLO ثم أعد فتح نفس الإيصال.</p>}
  </div>;
}
