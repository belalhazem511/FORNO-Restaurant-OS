  ...
  document: protectedProcedure
    ...
    .query(async ({ ctx, input }): Promise<TrustedPrintDocument> => {
      ...
      const document: TrustedPrintDocument = {
        ...
        restaurant: { name: { en: "SOLO Restaurant", ar: "مطعم SOLO" }, branch: { en: order.branch.name_en, ar: order.branch.name_ar }, address: { en: order.branch.address_en ?? "", ar: order.branch.address_ar ?? "" }, phone: order.branch.phone },
